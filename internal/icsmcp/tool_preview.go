...
func ToolInfos() []ToolInfo {
	return []ToolInfo{
		{Name: "upcoming_meetings", Description: "List ongoing and upcoming meetings by time. Compact output is default; pass detail=full for verbose fields. Descriptions are opt-in. Sort supports start_time, agenda, calendar, and ongoing_first.", Category: "read", ReadOnly: true, InputExample: `{"limit":10,"lookahead_days":30,"timezone":"","query":"","sort":"start_time","in_progress_only":false,"exclude_all_day":false,"exclude_cancelled":true,"include_description":false}`, DefaultArguments: map[string]any{"limit": 10, "lookahead_days": 30, "timezone": "", "query": "", "sort": "start_time", "in_progress_only": false, "exclude_all_day": false, "exclude_cancelled": true, "include_description": false}},
		{Name: "upcoming_meetings_by_calendar", Description: "List ongoing and upcoming meetings grouped by calendar. Compact output is default; limit applies per calendar. Sort applies within each calendar group.", Category: "read", ReadOnly: true, InputExample: `{"limit":10,"lookahead_days":30,"timezone":"","query":"","sort":"start_time","exclude_cancelled":true,"include_description":false}`, DefaultArguments: map[string]any{"limit": 10, "lookahead_days": 30, "timezone": "", "query": "", "sort": "start_time", "exclude_cancelled": true, "include_description": false}},
		{Name: "next_meeting", Description: "Return the next non-all-day, non-cancelled meeting.", Category: "read", ReadOnly: true, InputExample: `{"timezone":""}`, DefaultArguments: map[string]any{"timezone": ""}},
		{Name: "next_meetings", Description: "List upcoming meeting-focused events, excluding all-day and cancelled events, using compact output by default.", Category: "read", ReadOnly: true, InputExample: `{"limit":10,"lookahead_days":30,"timezone":"","sort":"start_time","include_description":false}`, DefaultArguments: map[string]any{"limit": 10, "lookahead_days": 30, "timezone": "", "sort": "start_time", "include_description": false}},
		{Name: "today_meetings", Description: "List meetings for the current display day. Defaults to agenda sort: ongoing timed meetings, upcoming timed meetings, then all-day or multi-day blocks.", Category: "read", ReadOnly: true, InputExample: `{"timezone":"","sort":"agenda","include_description":false}`, DefaultArguments: map[string]any{"timezone": "", "sort": "agenda", "include_description": false}},
		...
		{Name: "search_meetings", Description: "Search cached ongoing and upcoming meetings by title, calendar name, or cached description. Descriptions remain omitted from output unless include_description is true.", Category: "read", ReadOnly: true, InputExample: `{"query":"planning","limit":10,"sort":"start_time","exclude_cancelled":true}`, DefaultArguments: map[string]any{"query": "", "limit": 10, "sort": "start_time", "exclude_cancelled": true}},
		{Name: "free_busy", Description: "List busy blocks without meeting titles or descriptions. Use after and before for a specific availability window.", Category: "read", ReadOnly: true, InputExample: `{"after":"2026-06-30T15:00:00Z","before":"2026-07-01T00:00:00Z","limit":20,"exclude_cancelled":true,"sort":"start_time"}`, DefaultArguments: map[string]any{"limit": 20, "exclude_cancelled": true, "sort": "start_time"}},
