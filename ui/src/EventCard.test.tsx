...
import { AllDayGrid, EventCard } from './App';
...
describe('all-day event cards', () => {
  ...
  it('overlays the all-day disclosure on the last visible event row instead of adding a footer', () => {
    const meetings = ['One', 'Two', 'Three', 'Four'].map(name => ({ name, date: '2026-08-03', all_day: true }));
    const markup = renderToStaticMarkup(<AllDayGrid meetings={meetings} rangeStart="2026-08-03" days={1} calendarByID={new Map()} onSelect={() => undefined} timeFormat="12h"/>);

    expect(markup).toContain('grid-template-rows:repeat(3, 34px)');
    expect(markup).toContain('class="all-day-overflow" style="grid-column:1;grid-row:3"');
    expect(markup).toContain('aria-label="Show 1 additional all-day events for this day"');
  });
